  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => {
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Kerko Artikullin</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
            <Input
              placeholder="Kerko sipas emrit ose barkodit..."
              className="pl-10"
              autoFocus
              value={dialogSearch}
          ...
          <ScrollArea className="h-64">
            <div className="space-y-2">
              {filteredProducts.map((product) => (
                <div
                  key={product.id}
                  className={"p-3 border rounded-lg cursor-pointer transition-colors " + (product.current_stock > 0 ? "hover:bg-[#E0F7FA]" : "bg-gray-100 opacity-70")}
                  onClick={() => {
                    addToCart(product);
                    setDialogSearch('');
                  }}
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex justify-between items-center flex-1">
                      <div>
                        <p className="font-medium">{product.name || 'Pa emer'}</p>
                        <p className="text-sm text-gray-500">Barkod: {product.barcode || '-'}</p>
                      </div>
                      <div className="text-right">
                        <p className="font-bold text-[#00a79d]">{(product.sale_price || 0).toFixed(2)} EUR</p>
              ...
              {filteredProducts.length === 0 && dialogSearch.trim() && (
                <p className="text-center text-gray-400 py-8">Nuk u gjet asnje produkt</p>
              )}
              {filteredProducts.length === 0 && !dialogSearch.trim() && (
