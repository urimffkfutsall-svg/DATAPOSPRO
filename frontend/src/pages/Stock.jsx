...
const Stock = () => {
  ...
  const exportData = products.map(p => ({
    ...
    'Cmimi Blerjes': p.purchase_price || 0,
    'Cmimi Shitjes': p.sale_price || 0,
    ...
    try {
      ...
      for (const row of data) {
        await api.post('/products/import', {
          name: row['Emri'],
          barcode: row['Barkodi'] || '',
          purchase_price: row['Cmimi Blerjes'] || 0,
          sale_price: row['Cmimi Shitjes'] || 0,
          current_stock: row['Stoku Aktual'] || 0,
          unit: row['Njesia'] || 'cope',
        });
      }
      toast.success(data.length + ' produkte u importuan!');
      loadData();
    } catch (err) {
      toast.error('Gabim gjate importit te skedarit');
    }
  };
