...
// BookPRO API instance
export const bpApi = axios.create({
  baseURL: API,
  headers: { 'Content-Type': 'application/json' }
});

// Add token interceptor for bpApi
...
const AuthProvider = ({ children }) => {
  ...
  const login = async (username, password) => {
    try {
      ...
      // Don't show toast for subscription expired (402) - let the modal handle it
      if (status !== 402) {
        toast.error(message);
      }
...
const AppRoutes = () => {
  ...
  return (
    <Routes>
      ...
      {/* Gate Login - Faqja e autorizimit për superadmin */}
      <Route path="/gate" element={isAuthenticated ? <Navigate to="/login" /> : <GateLogin />} />
      
      {/* POS Login - Hapet direkt për PWA */}
      ...
      {/* BookPRO Routes - Hair Salon Management */}
      ...
      {/* HealthPRO Routes - Healthcare Institute Management */}
